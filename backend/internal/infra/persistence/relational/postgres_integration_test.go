...
func TestPostgresProviderLinksIntegration(t *testing.T) {
	dsn := os.Getenv("TEST_POSTGRES_DSN")
	if dsn == "" {
		t.Skip("TEST_POSTGRES_DSN is not configured")
	}
	...
	if err := accounts.ReconcileProviderLinks(ctx, web.ID); err != nil {
		t.Fatal(err)
	}
	...
	otherConsole, _, err := accounts.UpsertByIdentity(ctx, account.Credential{
	...
	if err := accounts.Delete(ctx, web.ID); err != nil {
		t.Fatal(err)
	}
	for _, id := range []uint64{build.ID, console.ID} {
		linked, getErr := accounts.Get(ctx, id)
		if getErr != nil {
